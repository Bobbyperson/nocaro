...
"""
    ...
    async def roulette(self, ctx):
        ...
        bg_vol = 1.0
        brokie_alert = False
        should_play_roulette_music = True
        ...
        async def track_manager():
            ...
            while ctx.guild.voice_client and ctx.guild.voice_client.is_connected():
                ...
                if not should_play_roulette_music:
                    break
...
Roulette will end when everyone leaves the VC, or when the original invoker types `endroulette`.
""")
        betting = True
        end = False
        # fmt: off
        ...
        while not end:
            ...
            yap_duration = 0
            if total_bets > 1:
                if wins > losses * 2 and rd.randint(1, 2) == 1:
                    _, yap_duration = await audio.play(
                        ctx,
                ...
                if losses > wins * 2 and rd.randint(1, 2) == 1:
                    if rd.randint(1, 5) == 1:
                        _, yap_duration = await audio.play(
                            ctx,
        ...
        should_play_roulette_music = False
