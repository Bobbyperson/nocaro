...
class Betting(commands.Cog):
        ...
        if self.bet_active:
            ...
            for i, option in enumerate(self.bet_options):
                try:
                    ...
                    val = 0
                embed.add_field(name=option, value=f"`{econ.unmoneyfy(val)}`", inline=True)
    ...
    async def do_payout(self, winner):
        ...
        for user_id, user_bet in winners.items():
            ...
            bet_percentage = user_bet / winner_bal
            winnings = bet_percentage * full_pot

            user = await self.bot.fetch_user(user_id)
            await econ.update_amount(user, winnings, False, "won bet")
            await user.send(f"Your bet on {winning_option} and won {econ.unmoneyfy(winnings)}")
